...
class BwcCombobox extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (this.hasAttribute('tag-limit'))
      this.#tagLimit = Number(this.getAttribute('tag-limit'))
    if (this.hasAttribute('object-key'))
      this.#key = this.getAttribute('object-key')
    if (this.hasAttribute('object-text'))
      this.#text = this.getAttribute('object-text')
    ...
    this.#elClearBtn.onclick = (e) => {
      ...
      if (!this.#multiple) {
        ...
        this.#selected = null
        this.dispatchEvent(
          new CustomEvent('select', { detail: this.#selected })
        )
      }
    }
      ...
      if (this.#multiple) {
        ...
        if (!found) {
          ...
          if (this.#allowCustomTag) {
            ...
            this.dispatchEvent(
              new CustomEvent('select', { detail: this.#tags })
            )
          }
        } else {
        ...
        if (!found) {
          ...
          if (this.#selected) {
            console.log(
              'onBlur - single select - not found and this.#selected truthy'
            )
            this.#selected = null
            this.dispatchEvent(
              new CustomEvent('select', { detail: this.#selected })
          ...
          if (!this.#selected) {
            console.log(
              'onBlur - single select - found and this.#selected falsy'
            )
            this.#selected = found
            this.dispatchEvent(
              new CustomEvent('select', { detail: this.#selected })
    ...
    this.required
      ? this.#elInput.setAttribute('required', '')
      : this.#elInput.removeAttribute('required')
    this.disabled
      ? this.#elInput.setAttribute('disabled', '')
  ...
  _matchItems(item1, item2) {
    ...
    return this._isStringType()
      ? item1 === item2
      : item1[this.#key] === item2[this.#key] ||
          item1[this.#text] === item2[this.#text]
  }
  _makeItemFromValue() {
    // TBD if all spaces only... return? trim white spaces?
    return this._isStringType()
      ? this.value
      : { [this.#key]: this.value, [this.#text]: this.value }
  }
  ...
  _addTag(item) {
    ...
    const itemExists = this.#tags.find((tag) =>
      this._isStringType()
        ? tag === item
        : tag[this.#key] === item[this.#key] &&
          tag[this.#text] === item[this.#text]
    )
  ...
  _updateTags() {
    ...
    this.#tags = tags.map((tag) =>
      this._isStringType()
        ? tag.innerText
        : { [this.#key]: tag.value, [this.#text]: tag.innerText }
    )
  ...
  _removeTag(span) {
    ...
    this._updateTags()
    if (!this._tagLimitReached() && !this.disabled)
      this.#elInput.removeAttribute('disabled')
  }
